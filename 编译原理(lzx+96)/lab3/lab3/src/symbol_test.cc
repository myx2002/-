...
int test_cond(node* temp)
{
    ...
    else
    {
        ...
        if(temp->left->right != NULL)
        {
            result = test_cond(temp->left->right);
            if(result == 1)
                return result;
        }
    }
    ...
    if(temp->type == "FUNC_EXP")
    {
        ...
        while(temp_table != NULL)
        {
            if(temp_table->var_def[temp->left->name] != NULL)
            {
                ...
                int index = 0;
                // for(int i=0;i<(int)temp_leaf->param.size();i++)
                //     cout<<temp_leaf->param[i]<<endl;
                while(temp_node != NULL)
                {
                    //保证参数个数一致
                    // cout<<index<<" ";
                    ...
                    temp_node->left->type == "IDENT" && (temp_node->left->left->type == "ARRAY" || temp_node->left->left->type == "INT"))
                    {
                        //获得当前维度
                        int array_size = 0;
                        //获得内层EXP
                        ...
                        while(temp_table_t != NULL)
                        {
                            ...
                            stoi(temp_leaf->param[index + 1]))
                            {
                                index++;
                                // cout<<index<<endl;
                                // cout<<temp_table_t->var_def[temp_node->left->name]->array_size.size()<<endl;
                                ...
                                //最后还有index++
                                index = index + stoi(temp_leaf->param[index]);
                                // cout<<index<<endl;
                ...
                if(index < (int)temp_leaf->param.size())
                {
                    cout<<"Error: line "<<temp->lineno<<": too few parameters"<<endl;
                    return 1;
                }
                break;
            }
