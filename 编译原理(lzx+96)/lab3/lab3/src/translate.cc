...
string array_ir(node* temp, int level)
{
    ...
    while(temp_node != NULL)
    {
        ...
        {
            array_now++;
            if(temp_node->left->type != "IDENT")
            {
                array_def_num.push_back(to_string(temp_node->left->value));
                // result += exp_ir(temp_node->left, 0);
                // array_def_num.push_back(return_design);
            }
            else if(temp_node->left->type == "IDENT" && temp_node->left->left->type == "ARRAY")
            {
                result += exp_ir(temp_node->left, 0);
                array_def_num.push_back(return_design);
    ...
    if(level == 1)
    {
        ...
        for(int i = 0;i < (int)array_def_num.size(); i++)
        {
            ...
            {
                result += ".WORD #";
                result += array_def_num[i] + "\n";
            }
            else
            {
                
                result += "x = " + now_design + " + #" + to_string(i * 4) + "\n";
                if(array_def_num[i].find('t') == string::npos)
                    result += print_tab(4) + "y =" + " #" + array_def_num[i] + "\n";
                else
                    result += print_tab(4) + "y =" + array_def_num[i] + "\n";
