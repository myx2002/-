...
class DiskManager {
 public:
  explicit DiskManager(const std::string &db_file);//explicit 防止隐式类型转换
 ...
 private:
  ...
  bool closed{false};
  char meta_data_[PAGE_SIZE];//page_size is the count of bytes. meta_data 转换成disk_file_meta_page
};
